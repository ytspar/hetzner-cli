...
export function getMainHelp(): string {
  return `<pre class="table-output">${bold("hctl")} ${dim(`v${HCTL_VERSION}`)} — Unified CLI for Hetzner Robot & Cloud API

${cyan("USAGE")}
  ${dim("$")} hctl ${dim("<command>")} ${dim("[options]")}
...
${cyan("EXAMPLES")}
  ${dim("$")} hctl auction list
  ${dim("$")} hctl auction list --max-price 50 --cpu epyc --sort ram
  ${dim("$")} hctl cloud server list
  ${dim("$")} hctl server list

Run ${yellow("hctl <command> --help")} for command-specific help.</pre>`;
}

export function getAuctionHelp(): string {
  return `<pre class="table-output">${bold("hctl auction")} — Browse Hetzner Server Auction

${cyan("COMMANDS")}
  ${green("list")} ${dim("(ls)")}        List auction servers with filters
  ${green("show")} ${dim("<id>")}        Show detailed server information
  ${green("status")}             Show auction data freshness

Run ${yellow("hctl auction list --help")} for filter options.</pre>`;
}

export function getAuctionListHelp(): string {
  return `<pre class="table-output">${bold("hctl auction list")} — List auction servers
...
${cyan("FILTERS")}
  ...
  ${green("--currency")} ${dim("<EUR|USD>")}  Hosted auction currency
...
${cyan("EXAMPLES")}
  ${dim("$")} hctl auction list --gpu
  ${dim("$")} hctl auction list --max-price 50 --sort ram --desc
  ${dim("$")} hctl auction list --cpu epyc --min-ram 256
  ${dim("$")} hctl auction list --datacenter FSN --disk-type NVMe</pre>`;
}

export function getCloudHelp(): string {
  return `<pre class="table-output">${bold("hctl cloud")} — Manage Hetzner Cloud resources
...
Run ${yellow("hctl cloud <resource> --help")} for more details.</pre>`;
}

export function getCloudServerHelp(): string {
  return `<pre class="table-output">${bold("hctl cloud server")} — Manage cloud servers
...
${cyan("EXAMPLES")}
  ${dim("$")} hctl cloud server list
  ${dim("$")} hctl cloud server describe web-prod-1
  ${dim("$")} hctl cloud server describe 42917324</pre>`;
}

export function getCloudNetworkHelp(): string {
  return `<pre class="table-output">${bold("hctl cloud network")} — Manage networks
...
export function getCloudFirewallHelp(): string {
  return `<pre class="table-output">${bold("hctl cloud firewall")} — Manage firewalls
...
export function getCloudVolumeHelp(): string {
  return `<pre class="table-output">${bold("hctl cloud volume")} — Manage volumes
...
export function getServerHelp(): string {
  return `<pre class="table-output">${bold("hctl server")} — Manage Robot dedicated servers
...
${cyan("EXAMPLES")}
  ${dim("$")} hctl server list
  ${dim("$")} hctl server get 1284751</pre>`;
}

export function getKeyHelp(): string {
  return `<pre class="table-output">${bold("hctl key")} — Manage SSH keys (Robot)
